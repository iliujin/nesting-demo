...
export function serializeSvg(layout: Layout, container: number, labels: boolean): string {
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>\n<svg xmlns="http://www.w3.org/2000/svg" viewBox="-2 -2 ${layout.width + 4} ${layout.height + 4}" width="1200" role="img"><title>${escapeXml(layout.name)} — 预计算示例 / 容器 ${container + 1}</title><desc>仅用于界面展示，不代表求解器性能。坐标单位为实例单位。</desc><rect width="${layout.width}" height="${layout.height}" fill="white" stroke="#334569" stroke-width="0.3"/><g transform="translate(0 ${layout.height}) scale(1 -1)">${polygons}</g>${text}</svg>`
}
